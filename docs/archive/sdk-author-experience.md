...
The owner-aligned AWS SDK for Python workflow is implemented and locally proved for S3, the official Kubernetes Python client is implemented as the second model-generated architecture, and official NATS Go and Python integrations now consume one language-neutral service mapping through two real profilers. All remain under upstream maintainer review. The cross-language NATS result proves the service-authority boundary for a handwritten SDK family, but its 892-line Go and 501-line Python overlays show that language-specific binding and state ergonomics still require substantial investigation.
...
The Kubernetes Python 36.0.3 proof starts from the authoritative Kubernetes v1.36.2 OpenAPI model retained by the SDK repository, follows the SDK's transformed generator input, and statically verifies the generated synchronous, asynchronous, and handwritten dynamic-client sources. It joins 908 generated endpoints to authoritative Kubernetes operations and emits 28 separate generator-injected dynamic endpoint records without asking maintainers to author or review an operation table. The exact extension digest, mapping, package, application, and historical evidence are under [`../authorship/kubernetes-python`](../authorship/kubernetes-python/).
...
The generated mapping contains 944 operation records, 1,875 public symbols, 132 explicit list-to-watch overrides, one source-verified `Watch.stream` condition delegation, one source-verified DynamicClient flow, eight distinct state-bound verb templates, and 95 built-in selectors generated from extension semantics. The rebuilt wheel adds 41,171 compressed bytes and one package-data declaration, and the profiler discovers the installed mapping without importing Kubernetes. The unchanged applications emit `get configmaps`, delegated `watch pods`, five separate DynamicClient ConfigMap lifecycle operations, and Dynamic Resource `watch pods`; the unmodeled CRD application emits nothing because its plural name and scope are not statically proved. A direct list remains `list`, a wrapped log stream remains `get pods/log`, and unresolved callables or selectors emit nothing. A mixed-source regression fixture preserves its existing declarative API and cache conditions while adding the mapped Kubernetes condition.

## Handwritten cross-language evidence

The official `nats.go` v1.53.1 and classic `nats-py` 2.15.0 integrations now consume the same 26-operation NATS service mapping and exact semantic digest. Go references all 26 canonical operations through 54 authoring rules that expand to 86 mapped call records; Python references 25 because its selected distribution exposes no distinct `consumer.update` method. Each language overlay owns only symbols, parameter or typed-field bindings, produced state, dependency identity, and language-specific delegation.

The Go conversion removed all authored Condition templates and YAML aliases without changing any of six real-profiler fixture profiles or the 86/64/36/0 mapped/excluded/deferred/unclassified source-surface result. It did not require a Go profiler change: the generator resolves canonical references into self-contained templates and retains both in generated output. This compatibility choice keeps service meaning in one authority while respecting the current profiler contract. The human-authored Go file decreased from 963 to 892 lines, demonstrating that semantic reuse is real but does not by itself solve SDK binding tedium.
...
The S3 review surface is one service-wide Smithy semantic overlay shared across languages, one four-entry botocore compatibility-alias file, one boto3 wrapper/factory annotation file, and one s3transfer behavior annotation file. The Kubernetes handwritten review surface is one `Watch.stream` delegation plus one DynamicClient flow describing two constructor aliases, one producer path, three selectors, three verification locations, seven methods, and nine fixed operation branches. NATS demonstrates a different burden: the service semantics are shared, but maintainers would still face 892 lines of Go-specific and 501 lines of Python-specific symbol, binding, and state input in the current prototypes. Maintainers do not inspect generated YAML mappings or built-in selector data line by line.
...
| New, removed, or reclassified API behavior | Extension stakeholders | Review the focused authoritative-source projection and any affected optional semantic supplement; publish a new immutable extension release only when adapter-facing vocabulary changes |
...
The local AWS proof installed rebuilt wheels directly by path, discovered mappings without importing SDK modules, and profiled seven unchanged application fixtures through the real command. Paired builds measured a current 10,441-byte compressed increase across the three distributions, including the generated semantic-digest fields. Five ordinary object-upload shapes emit `PutObject`, the runtime-selected service fixture emits nothing, and managed upload composes the five concrete operations reachable through its mapped runtime paths. The latter widening is approved for this experiment because it is actionable extension vocabulary and avoids subjective higher-level intent, but it is not presumed to be a universal cross-SDK rule. The Kubernetes proof does the same for five generated, wrapper, and DynamicClient applications with automatic installed discovery at a compressed cost of 41,171 bytes. NATS adds six unchanged applications in each of Go and Python, automatic package discovery, source-proven state, and separate dependency identities through their real profilers. Application source remains unchanged in every case.
...
1. Is this SDK-facing review surface acceptable, which existing teams would own SDK aliases, boto3 wrappers, and s3transfer behavior, and can every current service-level bridge entry be either derived or justified as a genuine source-model gap outside the SDK maintainer's responsibilities?
...
9. Are one source-verified `Watch.stream` delegation and one DynamicClient flow an acceptable representation of the handwritten package surface, and is the explicit CRD/live-discovery boundary understandable?
10. Would NATS maintainers accept canonical operation references plus focused source-surface review, and what tooling would reduce the remaining binding and state input enough for routine ownership?
...
| 2026-08-28 | Represent DynamicClient as a source-verified producer/state/method flow joined to a built-in selector catalog generated by extension automation. | Maintainers review one seven-method flow rather than a per-resource table; the profiler emits distinct fixed operations and remains silent when live discovery is the only source of plural name or scope. |
| 2026-08-28 | Keep DynamicClient scope resolution declarative in the SDK mapping. | Kubernetes-specific `cluster`, `namespaced`, and `all_namespaces` policy does not become hard-coded profiler behavior. |
| 2026-09-06 | Convert the NATS Go overlay from copied Condition templates and YAML aliases to canonical service-operation references shared with NATS Python. | Service meaning is maintained once and the profiler remains unchanged, but the modest reduction from 963 to 892 lines proves that language-specific binding and state ergonomics remain the larger maintainer burden. |
