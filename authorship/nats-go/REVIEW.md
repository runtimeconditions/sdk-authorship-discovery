...
NATS now has a separate language-neutral Service Operations Inventory with 26 stable operations, a current experimental projection supplement, and a generated service mapping. The inventory is the reviewed fallback service authority because this workflow has no adequate authoritative Smithy, OpenAPI, or equivalent NATS operation model. Both the Go and Python SDK integrations consume the same service mapping and exact semantic digest, and neither language overlay maintains canonical NATS Condition templates. The separate supplement is not an SDK-author obligation and remains subject to a field-by-field audit to determine whether the inventory and extension can support direct projection.
