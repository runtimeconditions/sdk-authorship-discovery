...
The current prototype generates the language-neutral S3 service mapping and immutable extension release from AWS's public Smithy model plus [`../../../extensions/aws-s3/model/service-operations-semantic-bridge.yaml`](../../../extensions/aws-s3/model/service-operations-semantic-bridge.yaml). That file is an experimental service-level supplement, not an AWS SDK-author requirement, and must be reduced to demonstrated Smithy gaps or removed. botocore then binds its versioned SDK surface to the canonical mapping.
...
- [`../../../extensions/aws-s3/model/service-operations-semantic-bridge.yaml`](../../../extensions/aws-s3/model/service-operations-semantic-bridge.yaml) currently supplies classifications, resource identity paths, roles, and secondary-resource decisions to the prototype compiler; each field must be tested for direct Smithy derivation before it is retained as a service-owned supplement.
