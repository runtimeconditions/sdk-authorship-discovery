...
class KubernetesPythonMappingGenerationTest(unittest.TestCase):
    ...
    def test_complete_surface_maps_to_exact_extension(self):
        self.assertEqual(self.mapping["metadata"]["operationCount"], 936)
        self.assertEqual(self.mapping["metadata"]["publicSymbolCount"], 1873)
        self.assertEqual(self.mapping["metadata"]["summary"]["operationRecords"], {"authoritative": 908, "dynamic": 28})
    ...
    def test_dynamic_methods_are_distinct_scalar_templates(self):
        dynamic = [item for item in self.mapping["operations"] if "conditionTemplate" in item]
