...
This project measures what the official Kubernetes Python maintainers would generate, annotate, package, and maintain. It consumes the exact source retained by Python client 36.0.3, the language-neutral Kubernetes service mapping generated by the current prototype from authoritative OpenAPI plus an experimental supplement, and the immutable Kubernetes API extension release. That supplement is a current compiler input, not a required SDK-author artifact; it must be reduced or removed wherever the OpenAPI source supports direct projection.
