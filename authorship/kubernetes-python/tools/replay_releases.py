...
def review_markdown(evidence: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "## Interpretation",
            "",
            f"The evidence separates three workloads that must not be collapsed: initial integration construction, which this replay does not measure; {summary['observedAutomationRepairs']} authored integration-tooling repair encountered during the replay; and zero per-operation semantic mapping edits observed in the replay. The final generator is backward-compatible with {summary['compatibleWithCurrentGenerator']} releases. If this integration lived in the SDK repository, its maintainers would need to review and ship an equivalent tooling change, even if Runtime Conditions contributors implemented it.",
            "",
